layui.use(['jquery','layer','table','form','laydate'],function () {
    ...
    $("#batchBtn").click(function () {
        ...
        if (data.length!=0){
            ...
            if (delBatchIf) {
                ...
                layer.msg("选中的订单有未结算的,不能删除。。",{icon:7,time:2000,shade:0.5,anim:6});
            }
        } else {
    ...
    function delBatchOrders(ids) {
        $.ajax({
            ...
            success:function (data) {
                if(data=='success'){
                    layer.msg("批量删除数据成功。。",{icon:1,time:2000,anim:4});
                    flush(currentPage); //刷新当前页
                }else {
                    layer.msg("批量删除数据失败！！",{icon:2,time:2000,anim:3});
                }
            },
            error:function () {
                layer.msg("服务器异常！！",{icon:3,time:2000,anim:3});
